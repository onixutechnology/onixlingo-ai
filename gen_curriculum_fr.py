#!/usr/bin/env python3
"""
Generate frontend/data/curriculum_fr.ts with 10 distinct level datasets.
Each level gets its own TEMAS data with unique, pedagogically-appropriate topics.
"""

LEVELS = [
    ("A1", "Bases y Supervivencia",  "Establece los cimientos del francés: saludos, números, rutinas básicas de oficina.",  "emerald", "a1"),
    ("A2", "Comunicación Cotidiana",  "Interactúa en situaciones elementales: pedidos, correos básicos, desplazamientos.",  "emerald", "a2"),
    ("B1", "Autonomía Profesional",   "Redacta correos formales, lidera juntas y expresa opiniones con soltura.",             "blue",    "b1"),
    ("B2", "Operaciones Ejecutivas",  "Domina la gestión de proyectos, equipos y presupuestos en francés de negocios.",       "blue",    "b2"),
    ("C1", "Liderazgo Estratégico",   "Lidera negociaciones de alto nivel, discursos corporativos y estrategia organizacional.", "orange", "c1"),
    ("C2", "Geopolítica Global",      "Domina retórica ejecutiva, geopolítica corporativa y toma de decisiones globales.",    "orange",  "c2"),
    ("TFI", "Certificación TFI®",     "Preparación intensiva para el examen oficial TFI® con ejercicios del formato real.",   "purple",  "tfi"),
    ("M1", "RRPP y Gestión de Crisis","Gestiona la comunicación de crisis, relaciones públicas y reputación de marca.",       "purple",  "m1"),
    ("M2", "Fusiones y Adquisiciones","Lidera integraciones corporativas y negociaciones de F&A transfronterizas.",           "purple",  "m2"),
    ("PRO", "Pro Executive: Gobernanza","Domina la supervisión del consejo, auditoría ética y sucesión ejecutiva global.",     "purple",  "pro"),
]
...
def generate_ts(temas_dict, levels):
    lines = []
    lines.append("import { LevelSection, LessonNode, ExerciseType } from './curriculum';")
    lines.append("")
    lines.append("// ================================================================")
    lines.append("// ONIXLINGO CURRICULUM FR - 10 NIVELES × 200 LECCIONES = 2,000 TOTAL")
    lines.append("// Cada nivel tiene su propio dataset de temas único y progresivo.")
    lines.append("// ================================================================")
    lines.append("")

    for (level_id, title, desc, color, prefix) in levels:
        var_name = f"TEMAS_{level_id.replace('-', '_')}_DATA"
        lines.append(f"const {var_name}: string[][] = [")
        for (topic, topic_desc, keyword) in temas_dict[prefix]:
            # Escape single quotes
            topic_esc = topic.replace("'", "\\'").replace("&", "&")
            topic_desc_esc = topic_desc.replace("'", "\\'").replace("&", "&")
            keyword_esc = keyword.replace("'", "\\'")
            lines.append(f"  ['{topic_esc}', '{topic_desc_esc}', '{keyword_esc}'],")
        lines.append("];")
        lines.append("")

    lines.append("// Generar lecciones de forma dinámica con tipado seguro")
    lines.append("const EXERCISE_TYPES: ExerciseType[] = ['lecture', 'grammar', 'chat', 'listening'];")
    lines.append("const POSITIONS: ('left' | 'center' | 'right')[] = ['center', 'left', 'center', 'right'];")
    lines.append("")
    lines.append("const LEVEL_PREFIXES: Record<string, string> = {")
    for (level_id, title, desc, color, prefix) in levels:
        lines.append(f"  '{prefix}': '{prefix}',")
    lines.append("};")
    lines.append("")
    lines.append("function buildLessons(prefix: string, rawData: string[][], limit = 200): LessonNode[] {")
    lines.append("  const lessons: LessonNode[] = [];")
    lines.append("  const total = rawData.length;")
    lines.append("  for (let idx = 0; idx < limit; idx++) {")
    lines.append("    const num = idx + 1;")
    lines.append("    const id = `fr-${prefix}-${num}`;")
    lines.append("    const type = EXERCISE_TYPES[idx % EXERCISE_TYPES.length];")
    lines.append("    const position = POSITIONS[idx % POSITIONS.length];")
    lines.append("    const dataItem = rawData[idx % total];")
    lines.append("    const progression = Math.floor(idx / total);")
    lines.append("    const progressionLabels = ['', ' — Práctica', ' — Aplicación', ' — Maestría'];")
    lines.append("    const suffix = progressionLabels[Math.min(progression, progressionLabels.length - 1)];")
    lines.append("    const title = `${dataItem[0]}${suffix}`;")
    lines.append("    lessons.push({")
    lines.append("      id,")
    lines.append("      title,")
    lines.append("      description: dataItem[1],")
    lines.append("      type,")
    lines.append("      locked: !(prefix === 'a1' && num === 1),")
    lines.append("      completed: false,")
    lines.append("      stars: 0,")
    lines.append("      position,")
    lines.append("      aiPrompt: `Contexte professionnel francophone : Discutez du sujet '${dataItem[0]}' en utilisant le vocabulaire clé '${dataItem[2]}' au niveau approprié.`,")
    lines.append("    });")
    lines.append("  }")
    lines.append("  return lessons;")
    lines.append("}")
    lines.append("")
    lines.append("export const CURRICULUM_FR: LevelSection[] = [")

    for (level_id, title, desc, color, prefix) in levels:
        var_name = f"TEMAS_{level_id.replace('-', '_')}_DATA"
        section_id = f"FR-{level_id}"
        lines.append("  {")
        lines.append(f"    id: '{section_id}',")
        lines.append(f"    title: 'Nivel {level_id}: {title}',")
        lines.append(f"    description: '{desc}',")
        lines.append(f"    color: '{color}',")
        lines.append(f"    lessons: buildLessons('{prefix}', {var_name}, 200)")
        lines.append("  },")

    lines.append("];")
    lines.append("")
    lines.append("// Helper para búsqueda rápida")
    lines.append("export function getFrenchLessonById(id: string): LessonNode | undefined {")
    lines.append("  const cleanId = id.toLowerCase();")
    lines.append("  for (const section of CURRICULUM_FR) {")
    lines.append("    const lesson = section.lessons.find(l => l.id.toLowerCase() === cleanId);")
    lines.append("    if (lesson) return lesson;")
    lines.append("  }")
...
output = generate_ts(TEMAS, LEVELS)

with open("frontend/data/curriculum_fr.ts", "w", encoding="utf-8") as f:
    f.write(output)

print(f"Generated curriculum_fr.ts with {output.count(chr(10))} lines.")
print(f"Levels: {len(LEVELS)} | Total lessons: {len(LEVELS) * 200}")
