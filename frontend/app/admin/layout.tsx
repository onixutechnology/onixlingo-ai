  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden relative">
      ...
      <div className="z-50 w-full flex justify-center pt-4 sm:pt-6 px-4 sm:px-6 flex-none relative">
        <header className="w-full max-w-[1800px] bg-white/40 backdrop-blur-xl border border-[#1d4ed8]  rounded-none overflow-visible ring-1 ring-[#1d4ed8] transition-all duration-300 transform-gpu relative">
          
          {/* Sci-fi accents on main header */}
          <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-[#1d4ed8] opacity-60"></div>
          <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-[#1d4ed8] opacity-60"></div>

          {/* Top bar with Logo, Title, and User Actions */}
          <div className="flex items-center justify-between px-5 sm:px-8 py-3 sm:py-4 bg-white/30 border-b border-[#1d4ed8] ">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-none bg-slate-900 border border-[#1d4ed8] flex items-center justify-center text-white shrink-0 ">
                  <span className="font-black font-mono text-[12px] tracking-widest uppercase">ON</span>
                </div>
                <div className="flex flex-col justify-center">
                  <span className="font-black font-mono uppercase tracking-widest text-lg text-slate-900 hidden sm:block  leading-none">ONIXLINGO PANEL ADMINISTRATIVO</span>
            ...
            <div className="flex items-center gap-3 sm:gap-5">
              <button 
                ...
                className="group flex items-center gap-2 px-4 py-2.5 bg-white/50 backdrop-blur-sm border border-[#1d4ed8] text-slate-700 hover:text-indigo-700 hover:border-[#1d4ed8] rounded-none text-xs font-black font-mono uppercase tracking-widest transition-all duration-300   hover:-translate-y-0.5 hover:-translate-x-0.5"
              >
              ...
              <div className="relative">
                <button 
                  ...
                  className={`relative p-2.5 bg-white/50 backdrop-blur-sm border border-[#1d4ed8] rounded-none text-slate-500 hover:text-indigo-600   transition-all duration-300 hover:-translate-y-0.5 hover:-translate-x-0.5 ${showNotifications ? 'bg-white text-indigo-600  translate-y-0 translate-x-0' : ''}`}
                >
                ...
                {showNotifications && (
                  <div className="absolute top-[calc(100%+12px)] right-[-50px] sm:right-0 w-80 bg-white/90 backdrop-blur-2xl border border-[#1d4ed8]  rounded-none z-50 animate-in fade-in slide-in-from-top-4 duration-300">
                    <div className="p-4 border-b border-[#1d4ed8] flex items-center justify-between bg-white/40">
                      ...
                      <button className="text-[9px] font-black font-mono uppercase tracking-widest px-2 py-1 bg-indigo-50 text-indigo-600 rounded-none hover:bg-indigo-100 transition-colors border border-[#1d4ed8]">CLEAR_ALL</button>
                    </div>
                    <div className="max-h-72 overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notif, idx) => (
                          <div key={idx} className="p-4 border-b border-[#1d4ed8] hover:bg-white/60 transition-colors cursor-pointer group">
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-none border border-[#1d4ed8] flex items-center justify-center shrink-0 ${notif.iconType === 'vip' ? 'bg-amber-50 text-amber-600 border-[#1d4ed8]' : 'bg-slate-100 text-slate-600'}`}>
              ...
              <div className="relative">
                <div 
                  ...
                  className={`w-10 h-10 sm:w-11 sm:h-11 rounded-none bg-slate-900 border border-[#1d4ed8] text-emerald-400 flex items-center justify-center font-black font-mono text-sm  cursor-pointer shrink-0 transition-all active:scale-95 hover:-translate-y-0.5 hover:-translate-x-0.5  ${showProfileMenu ? ' border-[#1d4ed8]' : ''}`}
                >
                  {userInitials}
                </div>
                {showProfileMenu && (
                  <div className="absolute top-[calc(100%+12px)] right-0 w-64 bg-white/90 backdrop-blur-2xl border border-[#1d4ed8]  rounded-none z-50 animate-in fade-in slide-in-from-top-4 duration-300">
                    <div className="p-5 border-b border-[#1d4ed8] flex items-center gap-4 bg-white/40">
                      <div className="w-10 h-10 rounded-none bg-slate-900 text-emerald-400 flex items-center justify-center font-black font-mono text-sm  shrink-0 border border-[#1d4ed8]">
                        {userInitials}
                      </div>
                      <div className="overflow-hidden">
                    ...
                    <div className="p-3 flex flex-col gap-1">
                      ...
                      <button className="w-full text-left px-4 py-2.5 text-[10px] uppercase tracking-widest text-rose-600 hover:bg-rose-50 rounded-none transition-all font-black font-mono flex items-center gap-3 border border-transparent hover:border-[#1d4ed8]" onClick={() => router.push('/')}>
          ...
          <div className="px-3 sm:px-6 py-2 bg-white/20 border-t border-[#1d4ed8]">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-none transition-all duration-300 whitespace-nowrap text-[10px] uppercase tracking-widest font-black font-mono ${
                      isActive 
                        ? 'bg-white/60 text-indigo-700  border border-white transform -translate-y-0.5' 
                        : 'text-slate-600 border border-transparent hover:bg-white/40 hover:text-slate-900  hover:border-[#1d4ed8] hover:-translate-y-0.5 hover:-translate-x-0.5 active:scale-95'
                    }`}
                  >
                    <Icon size={14} className={`shrink-0 transition-colors  ${isActive ? 'text-indigo-600' : 'text-slate-400'}`} />
                    {mod.label}
                  </button>
                );
              })}
