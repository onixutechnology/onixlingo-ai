...
const TIER_STYLES: Record<string, { bg: string; badge: string; glow: string }> = {
  Bronze:   { bg: 'bg-orange-50 border-orange-200',   badge: 'bg-orange-600 text-white',       glow: 'shadow-orange-200/50' },
  Silver:   { bg: 'bg-slate-50 border-slate-200',     badge: 'bg-slate-500 text-white',         glow: 'shadow-slate-200/50' },
  Gold:     { bg: 'bg-amber-50 border-amber-200',     badge: 'bg-amber-500 text-white',         glow: 'shadow-amber-200/50' },
  Platinum: { bg: 'bg-teal-50 border-teal-200',       badge: 'bg-teal-600 text-white',          glow: 'shadow-teal-200/50' },
  Obsidian: { bg: 'bg-slate-950 border-slate-800',    badge: 'bg-slate-700 text-slate-100',     glow: 'shadow-slate-900/60' },
};
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans selection:bg-teal-100 pb-20">
      ...
      <nav className="h-14 border-b border-slate-200 px-4 md:px-6 flex items-center justify-between bg-white shadow-none z-40 sticky top-0">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/dashboard')} className="p-2 hover:bg-slate-100 transition-all border border-transparent hover:border-slate-200 rounded">
            <ChevronLeft size={18} />
          </button>
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 bg-slate-950 flex items-center justify-center rounded-sm">
              <Trophy size={12} className="text-amber-500" />
            </div>
            <h1 className="font-black text-[10px] tracking-[0.3em] uppercase text-slate-900">
              Cognitive <span className="text-amber-600 font-serif italic">Achievements</span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex bg-slate-100 p-0.5 border border-slate-200">
          <button
            ...
            className={`px-3 md:px-4 py-1 text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'trophies' ? 'bg-slate-950 text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
          >
            🏆 Vitrina (1,000)
          </button>
          <button
            onClick={() => setActiveTab('ranking')}
            className={`px-3 md:px-4 py-1 text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'ranking' ? 'bg-slate-950 text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
          >
            📊 Ranking
          </button>
        </div>
            ...
            <motion.div key="trophies-view" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
              ...
              <div className="bg-white border border-slate-200 p-5 md:p-7 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-5">
                ...
                <div className="flex items-center gap-5">
                  <div className="w-14 h-14 bg-amber-50 border border-amber-200 flex items-center justify-center text-amber-600 shrink-0">
                ...
                <div className="w-full md:w-96 shrink-0">
                  <div className="flex justify-between mb-2 text-[10px] font-black uppercase tracking-widest text-slate-500">
                    ...
                    <span className="text-amber-600">{unlockedCount.toLocaleString()} / 1,000</span>
                  </div>
                  ...
                  <p className="text-[9px] font-black text-slate-400 mt-1.5 text-right">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  { label: 'Desbloqueados',  value: unlockedCount, suffix: 'trofeos', color: 'text-teal-600' },
                  { label: 'Bloqueados',      value: 1000 - unlockedCount, suffix: 'trofeos', color: 'text-slate-400' },
                  { label: 'XP Total',        value: userStats.total_xp?.toLocaleString() || 0, suffix: 'XP', color: 'text-amber-600' },
                  ...
                  <div key={s.label} className="bg-white border border-slate-200 p-4 text-center">
                    ...
                    <div className="text-[8px] font-black uppercase tracking-widest text-slate-400 mt-1">{s.suffix}</div>
                    <div className="text-[8px] font-bold text-slate-500 mt-0.5">{s.label}</div>
                  </div>
                ))}
              </div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-4 py-2 text-xs border border-slate-200 bg-slate-50 focus:outline-none focus:border-slate-400 font-medium"
                    />
                  </div>
                  <button
                    onClick={() => setShowFilters(!showFilters)}
                    className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 text-[9px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all"
                  >
                  ...
                  return (
                    <motion.div
                      ...
                      className={`border p-4 flex flex-col justify-between h-44 transition-all duration-200 relative overflow-hidden group ${
                        isUnlocked
                          ? `bg-white ${styles.bg.split(' ')[1]} hover:shadow-lg hover:${styles.glow} hover:-translate-y-0.5`
                          : 'bg-slate-50/80 border-slate-200 opacity-55 grayscale-[30%]'
                      } ${styles.bg}`}
                    >
                      {/* Tier badge + points */}
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className={`text-[11px] font-black uppercase tracking-tight mb-1 flex items-center gap-1.5 leading-tight ${
                          isUnlocked ? (trophy.tier === 'Obsidian' ? 'text-slate-200 group-hover:text-amber-400' : 'text-slate-900 group-hover:text-amber-600') : 'text-slate-400'
                        }`}>
                          {isUnlocked
                        ...
                        <p className={`text-[8px] font-bold uppercase leading-tight tracking-wide line-clamp-2 ${
                          trophy.tier === 'Obsidian' && isUnlocked ? 'text-slate-400' : 'text-slate-400'
                        }`}>
                          {trophy.description}
                        </p>
                      </div>
                      ...
                      <div className={`mt-3 pt-2.5 border-t flex justify-between items-center text-[7px] font-black uppercase tracking-widest ${
                        trophy.tier === 'Obsidian' ? 'border-slate-700' : 'border-slate-100'
                      }`}>
                        <span className={trophy.tier === 'Obsidian' && isUnlocked ? 'text-slate-500' : 'text-slate-400'}>
              ...
              {visibleCount < filteredTrophies.length && (
                <div className="flex justify-center pt-4">
                  <button
                    ...
                    className="flex items-center gap-2 px-8 py-3 bg-slate-950 text-white text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all active:scale-95"
                  >
                    <ChevronDown size={14} />
              ...
              {filteredTrophies.length === 0 && (
                <div className="py-20 text-center bg-white border border-dashed border-slate-300">
                  ...
                  {COUNTRIES.map(c => (
                    <button key={c.code} onClick={() => setActiveCountry(c.code)}
                      className={`px-3 py-1.5 text-[8px] font-black uppercase tracking-widest transition-all ${
                        activeCountry === c.code ? 'bg-slate-950 text-white' : 'text-slate-400 hover:text-slate-600'
                      }`}
                    >
                      {c.flag} {c.label}
                    </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                {isLoading ? (
                  ...
                  <div className="col-span-3 py-20 text-center bg-white border border-dashed border-slate-300">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Sin datos para este país todavía.</p>
                  </div>
                ) : (
                ...
                <div className="bg-white border border-slate-200 shadow-sm overflow-hidden mb-12">
                  ...
                  <div className="divide-y divide-slate-100">
                    {remaining.map((player, idx) => (
                      <div key={player.username + idx} className="grid grid-cols-[60px,1fr,100px,100px,120px] px-8 py-4 items-center hover:bg-slate-50 transition-colors group">
        ...
        <div className="bg-slate-900 p-8 flex flex-col md:flex-row items-center justify-between gap-6 border border-slate-800 mt-12">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-slate-400 text-[10px] font-bold uppercase tracking-tight max-w-sm">
