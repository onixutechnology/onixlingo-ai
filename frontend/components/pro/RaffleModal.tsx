...
export const RaffleModal = ({ onClose, totalTickets }: RaffleModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl shadow-black relative">
        {/* Decorative background glow */}
        <div className="absolute top-[-20%] left-[-10%] w-[300px] h-[300px] bg-amber-500/10 rounded-full blur-[80px] pointer-events-none" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[300px] h-[300px] bg-teal-500/10 rounded-full blur-[80px] pointer-events-none" />

        {/* HEADER */}
        <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 relative z-10">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-amber-500/10 rounded-xl text-amber-400 border border-amber-500/20 shadow-lg shadow-amber-500/5">
              <Ticket size={22} className="animate-pulse" />
            </div>
            <div>
              <h2 className="text-xl font-black text-white tracking-tight">OnixLingo Executive Raffle</h2>
              <p className="text-[10px] text-amber-400 font-bold uppercase tracking-widest mt-0.5">Titanium Member Privilege</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="text-slate-400 hover:text-white transition-colors p-2 bg-slate-800 rounded-full active:scale-95 border border-slate-700/50"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto relative z-10 scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {/* PROMOTIONAL BANNER */}
          <div className="relative overflow-hidden bg-gradient-to-r from-amber-600/20 to-teal-600/20 border border-amber-500/30 rounded-2xl p-6 text-center">
            <div className="absolute top-0 right-0 p-3 opacity-15">
              <Gift size={64} className="text-amber-400" />
            </div>
            <span className="inline-block px-3 py-1 bg-amber-400 text-slate-950 text-[9px] font-black uppercase tracking-widest rounded-full mb-3 shadow-md">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 text-center">
            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center justify-center gap-1.5">
            ...
            <div className="grid grid-cols-4 gap-2 max-w-xs mx-auto">
              {[
                ...
                <div key={idx} className="bg-slate-900 border border-slate-850 p-2.5 rounded-xl shadow-inner">
                  <span className="block text-xl font-black text-white font-mono leading-none">
                    {String(t.val).padStart(2, '0')}
                  </span>
                  <span className="block text-[8px] text-slate-500 font-extrabold mt-1 tracking-wider">
                    {t.label}
                  </span>
                </div>
              ))}
          ...
          <div>
            <div className="flex justify-between items-end mb-3">
              ...
              <span className="text-sm font-black text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-0.5 rounded-full">
                {totalTickets} {totalTickets === 1 ? 'Boleto' : 'Boletos'}
              </span>
            </div>

            {totalTickets === 0 ? (
              <div className="bg-slate-950 border border-slate-850 rounded-2xl p-8 text-center">
                <Ticket size={36} className="text-slate-700 mx-auto mb-3 opacity-40" />
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">No tienes boletos acumulados aún</p>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[160px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-800">
                  {ticketList.map((ticket, idx) => (
                    <motion.div 
                      ...
                      className="bg-gradient-to-b from-amber-500/5 to-amber-500/10 border border-amber-500/20 rounded-lg p-2.5 flex flex-col justify-center items-center relative overflow-hidden shadow-sm"
                    >
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-3 bg-slate-950 rounded-r-full border-r border-amber-500/20" />
                      <div className="absolute right-0 top-1/2 -translate-y-1/2 w-1.5 h-3 bg-slate-950 rounded-l-full border-l border-amber-500/20" />
          ...
          {/* RULES INFO */}
          <div className="flex gap-3 bg-slate-950/40 p-4 border border-slate-850 rounded-2xl text-[11px] text-slate-400 font-semibold leading-relaxed">
