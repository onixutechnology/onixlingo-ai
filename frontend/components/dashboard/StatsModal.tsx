...
export function StatsModal({ onClose, userStats }: StatsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200 font-sans">
      <div className="bg-white border border-slate-200 p-6 md:p-8 max-w-4xl w-full shadow-2xl relative animate-in zoom-in-95 duration-200 rounded-none overflow-hidden flex flex-col md:flex-row gap-8">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 rounded-full p-2 transition-all"
        >
          <X size={18} />
        </button>
        ...
        <div className="flex-1 border-r border-slate-100 pr-0 md:pr-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Racha Actual: {userStats.streak} Días</p>
            </div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 p-4 mb-4">
            <div className="flex items-center justify-between mb-4 border-b border-slate-200 pb-2">
              ...
              <span className="text-[9px] bg-rose-100 text-rose-700 px-2 py-0.5 font-bold uppercase border border-rose-200">En Racha</span>
            </div>
            <div className="grid grid-cols-7 gap-2">
              {['L', 'M', 'X', 'J', 'V', 'S', 'D'].map((d) => (
                <div key={d} className="text-center text-[8px] font-black text-slate-400">{d}</div>
              ))}
                ...
                return (
                  <div 
                    ...
                    className={`aspect-square flex items-center justify-center text-[10px] font-bold border transition-colors ${
                      isToday ? 'bg-rose-500 text-white border-rose-600 shadow-md ring-2 ring-rose-200' :
          ...
          <div className="bg-slate-900 p-4 border-l-4 border-rose-500 flex items-start gap-3">
            <TrendingUp size={16} className="text-rose-500 mt-0.5" />
            <p className="text-[9px] text-slate-300 font-medium leading-relaxed">
              Mantener una racha activa incrementa tus multiplicadores de XP diarios. Estás en el <strong className="text-white">Top 15%</strong> de usuarios corporativos más consistentes.
            </p>
          </div>
        </div>
        ...
        <div className="flex-1">
          {userStats.level_details && (
            <div className="bg-slate-900 border border-slate-800 p-5 mb-6 rounded-none relative overflow-hidden shadow-lg">
              <div className="absolute -right-6 -top-6 text-slate-800 opacity-50 rotate-12">
                <Award size={100} />
              </div>
              <div className="flex justify-between items-end mb-3 relative z-10">
                <div>
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Nivel Actual</h4>
                  <p className="text-3xl font-black text-amber-400 leading-none mt-1">{userStats.level_details.level}</p>
                </div>
                <div className="text-right">
                  <h4 className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Nivel {userStats.level_details.next_level}</h4>
                  <p className="text-[10px] font-black text-white mt-1 tracking-wider">Faltan {userStats.level_details.xp_to_next.toLocaleString()} XP</p>
                </div>
              </div>
              <div className="w-full bg-slate-800 h-2 mt-3 relative z-10 border border-slate-700">
          ...
          <div className="flex items-center gap-3 mb-4 mt-6 md:mt-0">
            <div className="bg-amber-50 p-2.5 rounded-full border border-amber-100">
              <Zap size={20} className="text-amber-500" />
            </div>
            <div>
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-900">Historial de Experiencia</h3>
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Total Acumulado: {userStats.xp.toLocaleString()} XP</p>
            </div>
          </div>

          <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
            {xpHistory.length === 0 ? (
              <p className="text-xs text-slate-400 font-semibold p-4 text-center border border-dashed border-slate-200">No hay actividad reciente.</p>
            ) : (
              xpHistory.map((log: any, idx: number) => (
                <div key={idx} className="bg-white border border-slate-200 p-3 flex items-center justify-between hover:border-amber-300 transition-colors group">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-amber-50 group-hover:text-amber-500 transition-colors">
                      <Zap size={14} />
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-slate-800 uppercase tracking-wider">{log.module}</p>
                      <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{formatDate(log.date)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black text-amber-500">+{log.amount}</span>
                    <span className="text-[8px] font-bold text-slate-400 ml-1">XP</span>
                  </div>
                </div>
              ))
            )}
