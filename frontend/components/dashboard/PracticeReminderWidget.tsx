...
export default function PracticeReminderWidget({ themeColor = 'orange' }: PracticeReminderWidgetProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const storedTime = localStorage.getItem('practice_reminders_time') || '20:00';
    ...
    if (nextState) {
      ...
      if ('Notification' in window) {
        ...
        if (permission === 'granted') {
          ...
          localStorage.setItem('practice_reminders_time', reminderTime);
          setEnabled(true);
          setStatusMessage('¡Recordatorios activados!');
          
          // Enviar notificación de prueba de inmediato
          try {
            new Notification('OnixLingo', {
              body: `¡Felicidades! Tus recordatorios diarios de práctica se programaron para las ${reminderTime}.`,
              icon: '/favicon.ico'
            });
          } catch (e) {
  ...
  const handleTimeChange = (time: string) => {
    ...
    if (enabled) {
      localStorage.setItem('practice_reminders_time', time);
      setStatusMessage(`Hora cambiada a las ${time}`);
  ...
  return (
    <div 
      className={isWood 
        ? `border ${colors.border} p-5 rounded-xl shadow-[0_10px_40px_rgba(14,165,233,0.08)] flex flex-col justify-between relative overflow-hidden group text-[#ecd3b5]`
        : `bg-white border ${colors.border} p-5 rounded-xl shadow-[0_10px_40px_rgba(14,165,233,0.08)] flex flex-col justify-between relative overflow-hidden group`}
    ...
    >
      ...
      <div className="relative z-10 space-y-4">
        <div className="flex justify-between items-start">
          <div>
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className={`text-[8px] font-black uppercase tracking-[0.2em] ${isWood ? 'text-amber-200/50' : (themeColor === 'orange' ? 'text-orange-600' : 'text-sky-600')}`}>Hábitos Diarios</span>
            </div>
            ...
            <p className={`text-[9px] font-semibold leading-none mt-1.5 ${isWood ? 'text-sky-500' : (themeColor === 'orange' ? 'text-orange-700' : 'text-sky-700')}`}>
          ...
          <button 
            ...
            className={`p-2 transition-all ${
              enabled 
                ...
                : isWood 
                  ? 'bg-[#361d0f] text-amber-500/60 hover:bg-[#462614] hover:text-amber-400' 
        ...
        <div className={`flex items-center gap-3 p-2.5 border rounded-xl justify-between ${
          ...
          <div className={`flex items-center gap-1.5 text-[10px] font-bold ${isWood ? 'text-[#ecd3b5]/80' : (themeColor === 'orange' ? 'text-orange-800' : 'text-sky-800')}`}>
            <Clock size={12} className={isWood ? 'text-amber-500/60' : (themeColor === 'orange' ? 'text-orange-600' : 'text-sky-600')} />
          ...
          <input 
            ...
            className={`px-2 py-1 text-xs font-bold font-mono outline-none rounded-xl w-20 ${
              isWood 
        ...
        {statusMessage && (
          <div className="text-[8px] font-black uppercase tracking-widest text-emerald-600 animate-pulse">
            {statusMessage}
          </div>
        )}
      </div>
