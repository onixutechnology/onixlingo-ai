...
export default function LessonNode({ data, color, onClick }: Props) {
  ...
  const colorMap: Record<string, string> = {
    emerald: 'bg-emerald-500 shadow-emerald-600 border-emerald-400',
  ...
  const baseColor = colorMap[color] || 'bg-gray-500 shadow-gray-600';
  
  // Estilos dinámicos
  const bgClass = isLocked ? 'bg-slate-200 shadow-slate-300 border-slate-200' : baseColor;
  const textClass = isLocked ? 'text-slate-400' : 'text-white';
  ...
  return (
    <div className={`flex flex-col items-center relative mb-8 z-10 ${
      ...
      >
        ...
        {isLocked ? (
          ...
          <Trophy className="w-9 h-9 text-yellow-300 drop-shadow-md" />
            ...
            <Star className="w-10 h-10 fill-yellow-400 text-yellow-600 drop-shadow-sm" />
        ) : (
        ...
        {!isLocked && data.stars > 0 && (
            <div className="absolute -top-6 flex gap-1 bg-white/90 px-2 py-1 rounded-full shadow-sm border border-slate-100">
                {[...Array(3)].map((_, i) => (
      ...
      <div className="mt-3 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-xl shadow-sm border border-slate-200 font-bold text-slate-600 text-sm text-center max-w-[160px] transform hover:scale-105 transition-transform cursor-default">
        {data.title}
      </div>
    </div>
  );
